...
// Main image generation using Pollinations.ai
...
module.exports = async (req, res) => {
    ...
    if (path === '/' || path === '/api') {
        return res.status(200).json({
            ...
            baseURL: 'https://tnehgen.edgeone.app',
            endpoints: {
                'GET /': 'API Information',
                'POST /api/create-key': 'Create API Key (expireDate required)',
                'GET /api/gen': 'Generate Image (JSON response)',
                'GET /api/gen/image': 'Generate Image (Direct image)',
                'GET /api/keys': 'List all API Keys',
                'DELETE /api/revoke-key': 'Revoke API Key'
            },
            example: {
                createKey: 'POST /api/create-key -d {"expireDate": "2026-12-31"}',
                generateImage: 'GET /api/gen?apikey=YOUR_KEY&prompt=cat',
                directImage: 'GET /api/gen/image?apikey=YOUR_KEY&prompt=dragon'
            }
        });
    }
    
    // POST /api/create-key - Create new API key
    if (path === '/api/create-key' && req.method === 'POST') {
        ...
        if (!isValidDate(expireDate)) {
            return res.status(400).json({ 
                error: 'Invalid expireDate. Must be future date in YYYY-MM-DD format'
            });
        }
        ...
        return res.status(201).json({
            ...
            endpoint: `https://tnehgen.edgeone.app/api/gen?apikey=${apiKey}&prompt=your_prompt`
        });
    }
    ...
    // GET /api/gen/image - Generate image (direct image response)
    ...
    return res.status(404).json({ 
        ...
        availableEndpoints: ['/', '/api', '/api/create-key', '/api/gen', '/api/gen/image', '/api/keys', '/api/revoke-key']
    });
};
